...
class MazeEnv(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    ...
    def step(self, action):
        ...
        if self.reward_type == 'sparse':
            reward = 1.0 if np.linalg.norm(ob[:4] - self._target) <= 0.5 else 0.0
        elif self.reward_type == 'dense':
            reward = np.exp(-np.linalg.norm(ob[:4] - self._target))
        else:
    ...
    def _get_obs(self):
        ...
        return np.concatenate([obs, self._target]).ravel()
    ...
    def set_target(self, target_location=None):
        ...
        self._target = (self._target[0],self._target[1], 0, 0)
        # add goal space = state space
    
    def set_target_new(self, location):
